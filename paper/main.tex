...
Conventional LWFS approaches achieve increased local synthesis accuracy by the direct spatial bandlimitation of the WFS driving functions \cite{Winter2016-TASL, Hahn16:AES, Hahn17:EUSIPCO, hahn2022cylindrical}.
...
The present contribution discussed a novel local Wave Field Synthesis approach, relying on a filter transformation that allows derivation of a temporal filter bank from an arbitrary spatial filter.
By transforming spatial antialiasing filters to a temporal filter bank, antialiasing can be performed by simple temporal bandlimitation of the SSD driving signals.
With the above antialiasing strategy local WFS can be implemented by choosing an appropriate spatial antialiasing filter based on a predefined reference listening position.

It was demonstrated via numerical simulations that the approach is capable of the suppression of aliasing wavefronts along a straight line/ray (containing the reference position) with its direction of adjusted by the center wavenumber of the antialiasing spatial band-pass filter.
The performance of the presetned approach was compared with LWFS by direct modal bandlimitation as a direct, reference solution.
It was shown, that temporal LWFS gives a feasible approximation for the reference, while being a more robust approach, with less computational demand, allowing real-time implementations. 
